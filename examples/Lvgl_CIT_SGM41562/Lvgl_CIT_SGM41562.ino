...
void my_touchpad_read(lv_indev_drv_t *indev_drv, lv_indev_data_t *data)
{
    if (CST816T->IIC_Interrupt_Flag == true)
    {
        ...
        if ((uint32_t)CST816T->IIC_Read_Device_Value(CST816T->Arduino_IIC_Touch::Value_Information::TOUCH_COORDINATE_X) == 60 &&
            (uint32_t)CST816T->IIC_Read_Device_Value(CST816T->Arduino_IIC_Touch::Value_Information::TOUCH_COORDINATE_Y) == 150)
        {
        }
        else
        {
...
void setup()
{
    ...
    Serial.println("firmware: [TQT-C6_V1.2][Lvgl_CIT_V1.2]_firmware_V1.0.4");
...
void loop()
{
    ...
    case CIT_UI.Window_Current_State::Window_WIFI_STA_Test: // WIFI_STA_Test
        ...
        break;
    case CIT_UI.Window_Current_State::Window_IMU_Test:  // IMU_STA_Test
        if (CIT_UI.Window_Initialization_Flag == false) // 初始化
        {
            Window_IMU_Test_Initialization();
            Window_CycleTime1 = millis() + 500; // 500ms
        }
        else
        {
